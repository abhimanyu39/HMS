...
app.get("/getAllPatients", (req, res) => {
  let statement = `SELECT * FROM patient INNER JOIN patientsfillhistory ON patient.email = patientsfillhistory.patient INNER JOIN medicalhistory ON patientsfillhistory.history = medicalhistory.id`;
...
//get all apppointments of each patient
app.get("/getAllAppointment", (req, res) => {
  console.log(req.query);
  let statement = `SELECT * FROM appointments WHERE appointments.email = ${req.query.email}`;
...
app.get("/getAppt", (req, res) => {
  let params = req.query;
  let apptId = params.id;
  let statement =
    "SELECT * FROM appointments WHERE appointments.id = " + apptId;
...
app.get("/editAppointment", (req, res) => {
  ...
  let statement = `UPDATE Appointments SET doctor="${doctor}", date="${date}", starttime="${starttime}", endtime="${endtime}", concerns="${concerns}", symptoms="${symptoms}", diagnosis="${diagnosis}", prescription="${prescription}" WHERE appointments.id=${appt_id};`;
  console.log(statement);
  con.query(statement, function (error, results, fields) {
    if (error) throw error;
    else {
...
app.get("/schedule", (req, res) => {
  ...
  let sql_try = `INSERT INTO Appointments (email, date, starttime, endtime,concerns, symptoms, doctor, status) 
                 VALUES (${`"${patientEmail}"`},${sql_date}, ${sql_start}, ${sql_end},${`"${concerns}"`}, ${`"${symptoms}"`},${`"${doctor}"`}, "NotDone")`;
  console.log(sql_try);
  con.query(sql_try, function (error, results, fields) {
    if (error) throw error;
    else {
...
app.get("/deleteAppt", (req, res) => {
  let a = req.query;
  let uid = a.uid;
  let statement = `DELETE FROM Appointments WHERE id=${uid};`;
  console.log(statement);
  con.query(statement, function (error, results, fields) {
    if (error) throw error;
    else {
...
app.get("/deletePatient", (req, res) => {
  ...
  con.query(statement, function (error, results, fields) {
    if (error) throw error;
      ...
      statement = `DELETE FROM Appointments WHERE email="${email}";`;
      con.query(statement, function (error, results, fields) {
        if (error) throw error;
        else {
          return res.json({
